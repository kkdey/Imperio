...
if(!dir.exists(paste0(output_dir, "/TSS"))){
  dir.create(paste0(output_dir, "/TSS"))
}
...
for(gg in genes_in_chr){
  ...
  ###  Get starts and ends for TSS+-5kb regions for each gene  gg  #######################
...
save(merge_df, file = paste0(output_dir, "/TSS/chr_", numchr, ".rda"))
