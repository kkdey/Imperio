...
if(!dir.exists(paste0(output_dir, "/Roadmap"))){
  dir.create(paste0(output_dir, "/Roadmap"))
}

geneanno = data.table::fread("../data/geneanno.csv")
abc_per_gene = get(load("../data/Roadmap_per_gene_beds_Blood.rda"))
...
for(gg in genes_in_chr){
  ...
  ########## which SNPs fall in the ABC or Roadmap bed regions  ######################
...
rownames(merge_df) = geneanno$symbol[match(genes_in_chr, geneanno$id)]

save(merge_df, file = paste0(output_dir, "/Roadmap/chr_", numchr, ".rda"))
