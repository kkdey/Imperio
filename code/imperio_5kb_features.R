...
if(!dir.exists(paste0(output_dir, "/5kb"))){
  dir.create(paste0(output_dir, "/5kb"))
}
...
for(gg in genes_in_chr){
  ...
  ###  Get starts and ends for 5kb regions for each gene  gg  #######################
...
save(merge_df, file = paste0(output_dir, "/5kb/chr_", numchr, ".rda"))
