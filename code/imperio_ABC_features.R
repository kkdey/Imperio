...
if(!dir.exists(paste0(output_dir, "/ABC"))){
  dir.create(paste0(output_dir, "/ABC"))
}
...
for(gg in genes_in_chr){
  ...
  ##########which SNPs fall in the ABC  bed regions  ######################
...
save(merge_df, file = paste0(output_dir, "/ABC/chr_", numchr, ".rda"))
